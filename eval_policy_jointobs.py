...
actor2.net.load_state_dict(torch.load("/home/lzeng/Thinclab Code/HVT/IA2C/New_Obs_Old_Rew/best_act2")) #BiB: Also select def/att @ bottom
...
for ep in range(NUM_EPISODES):
    ...
    a1 = torch.tensor(0)#actor1.sample_action( o1 )
    ...
    for step in range(STEPS_PER_EPISODE):
        ...
        a1_ = torch.tensor(0)#actor1.sample_action( o1_)
        ...
        if step == STEPS_PER_EPISODE - 1:
            print(f'Stop in episode {ep}, step {step}: {done}. Rewards={r}')
...
for k in  envs.logger.logs.keys():
    filename = f"logfile_{k}_att" #BiB: change def/att based on whose best policy was loaded
