...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        # TODO: Fix when not using any adj_matrix
        ...
        if self.use_adj_matrix:
            if self.use_torch:
                ...
                self.adj_mask = torch_ones((self.num_predators, self.num_predators))
            elif self.use_tf:
                from tensorflow import ones as tf_ones
                self.adj_mask = tf_ones((self.num_predators, self.num_predators))
            else:
    ...
    def observation(self, world):
        ...
        elif self.use_hyperedge_index:
            return np.array(obs_n, dtype=np.float32), self.get_agent_hyperedge_index(world), np.array(available_actions_n), \
                self.grid.reshape(self.state_size)
        else:
    ...
    def get_agent_proximal_adj_matrix(self, world):
        ...
        if self.use_torch:
            ...
            if self.only_binary_adj_matrix:
                return torch.unsqueeze(adj_matrix, 0)
            else:
                if self.use_self_attention:
                    # Laplacian modified norm
                    sparse_norm = normalize_adj(adj_matrix.numpy())
                else:
                    # Laplacian modified norm
                    sparse_norm = normalize_adj(adj_matrix.numpy() + np.eye(self.num_predators))

                normalized_adj_matrix = sparse_mx_to_torch_sparse_tensor(sparse_norm).to_dense()
            ...
            if self.only_binary_adj_matrix:
                return tf.unsqueeze(adj_matrix, 0)
            else:
                if self.use_self_attention:
                    # Laplacian modified norm
                    sparse_norm = normalize_adj(adj_matrix.numpy())
                ...
                normalized_adj_matrix = tf.sparse.to_dense(sparse_mx_to_tf_sparse_tensor(sparse_norm))
    ...
    def get_agent_fixed_adj_matrix(self):
        ...
        # Laplacian modified norm
        sparse_norm = normalize_adj(self.fixed_adjacency_matrix.numpy() + np.eye(self.num_predators))
        ...
        if self.use_torch:
            ...
            if self.use_self_attention:
                self.fixed_adjacency_matrix = self.fixed_adjacency_matrix + torch.eye(self.num_predators)
            ...
            self.fixed_adjacency_matrix_normalized = sparse_mx_to_torch_sparse_tensor(sparse_norm).to_dense()
            ...
            if self.use_self_attention:
                self.fixed_adjacency_matrix = self.fixed_adjacency_matrix + tf.eye(self.num_predators, self.num_predators)
            ...
            self.fixed_adjacency_matrix_normalized = tf.sparse.to_dense(sparse_mx_to_tf_sparse_tensor(sparse_norm))
    ...
    def get_agent_adj_matrix(self, world):
        if not self.use_fixed_graph:
            ...
            elif self.capture_freezes and self.adj_mask_changed:
                self.get_agent_fixed_adj_matrix()
                self.adj_mask_changed = False
            else:
                pass

            return (self.fixed_adjacency_matrix, self.fixed_adjacency_matrix_softmax,
                    self.fixed_adjacency_matrix_normalized)
    ...
    def get_agent_hyperedge_index(self, world):
        ...
        if self.use_torch:
            ...
            from multiagent_particle_env.cg_utils import adj_matrix_to_hyperedge_index
